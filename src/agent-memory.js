// BODH Cognitive Memory Architecture (BCMA) - Agent Memory Manager
// Implements 4-Tier Memory Infrastructure:
// 1. Working Memory (Structured Academic State Snapshot)
// 2. Episodic Memory (Rolling Conversation Ledger)
// 3. Cognitive Profile (Misconception & Skill Tracking)
// 4. Procedural Directives & State Machine Integration

import { normalizeSubject, extractChapterNum, normalizeBoard, formatTag } from "./agent-tools.js";
...
export class MemoryManager {
  static reconcile(userMessage, pastHistory = [], incomingState = null) {
    ...
    if (!state.subject_id || !state.chapter_num) {
      for (const turn of pastHistory.slice().reverse()) {
        ...
        if (!state.chapter_num) {
          const ch = extractChapterNum(text);
    ...
    if (!isMetaDebate && !isAnswering) {
      const explicitChapter = extractChapterNum(cleanMsg);
      if (explicitChapter) {
    ...
    } else if (/রোডম্যাপ|কমন|৮০\/২০|80\/20|গুরুত্বপূর্ণ|অধ্যায়গুলো/i.test(cleanMsg)) {
